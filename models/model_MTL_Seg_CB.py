...
class MTLNet(nn.Module):
    def __init__(self):
        ...
        self.dbconv =  double_conv(128, 64, 0.2)
        self.fc_out = torch.nn.Sequential(torch.nn.Linear(64, 32), torch.nn.ReLU(), torch.nn.Linear(32, 2))
    ...
    def forward(self, input):
        ...
        m_seg_intermedio = m_seg[2]
        m_seg = m_seg[0]
        
        m_class = self.dbconv(m_seg_intermedio)
        m_class = F.adaptive_avg_pool2d(m_class, (1, 1))
        m_class = torch.flatten(m_class, 1)
        m_class = self.fc_out(m_class)
