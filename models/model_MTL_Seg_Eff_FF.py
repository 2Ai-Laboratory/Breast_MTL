...
class SegResNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x, x_features = self.decode(x, down_x)
        return x
...
class SegResNetVAE(SegResNet):
    ...
    def forward(self, x):
        ...
        vae_input = x
        x2, x_features = self.decode(x, down_x)

        if self.training:
            vae_loss = self._get_vae_loss(net_input, vae_input)
            return x2, vae_loss, x, x_features

        return x2, None, x, x_features
...
class MTLNet(nn.Module):
    ...
    def forward(self, input):
        ...
        m_seg_label = m_seg[0]
        m_seg_features = m_seg[3]
        
        input_features = torch.cat((m_seg_features,m_seg_label), 1)
      
        m_class = self.classification(input_features)

        return m_seg_label, m_class